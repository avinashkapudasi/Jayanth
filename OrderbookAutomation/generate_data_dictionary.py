...
def detect_header_row(workbook_path: Path, sheet_name: str) -> int:
    ...
    best_index = 0
    best_score = -10**9
    for index, row in enumerate(worksheet.iter_rows(min_row=1, max_row=min(worksheet.max_row or 1, 25), values_only=True)):
...
def load_sheet_info(workbook_path: Path, sheet_name: str) -> SheetInfo | None:
    header_row = detect_header_row(workbook_path, sheet_name)
    raw_headers = pd.read_excel(workbook_path, sheet_name=sheet_name, header=None, skiprows=header_row, nrows=0)
    headers = [safe_text(value) for value in list(raw_headers.columns)]
    headers = [header if header else f"Column {index + 1}" for index, header in enumerate(headers)]

    if not headers:
        return None

    dataframe = pd.read_excel(workbook_path, sheet_name=sheet_name, header=None, skiprows=header_row + 1, names=headers)
    dataframe = dataframe.loc[:, ~dataframe.columns.duplicated()].copy()
    return SheetInfo(workbook_path.name, sheet_name, headers, dataframe)
...
def infer_dtype(series: pd.Series, column_name: str) -> str:
    ...
    normalized = [safe_text(value) for value in values]
    lower_name = safe_text(column_name).lower()

    bool_values = {"y", "n", "yes", "no", "true", "false", "0", "1"}
    if all(item.lower() in bool_values for item in normalized):
        return "Boolean"
    ...
    if any(token in lower_name for token in ["date", "eta", "expiry", "expiration", "req"]):
        parsed_dates = pd.to_datetime(pd.Series(normalized), errors="coerce")
...
def recommended_join_keys(workbook_name: str, sheet_name: str, headers: list[str]) -> str:
...
def format_table_row(values: dict[str, object]) -> str:
    escaped = {key: safe_text(value).replace("|", "\\|") for key, value in values.items()}
    return "| {Workbook Name} | {Worksheet Name} | {Exact Column Name} | {Data Type (inferred)} | {Sample Value} | {Nullable} | {Duplicate Column Names} | {Recommended Join Key(s)} |".format(
        **escaped
    )
...
def main() -> None:
    ...
    for workbook_path in WORKBOOKS:
        ...
        workbook_counts[workbook_path.name] = workbook_counts.get(workbook_path.name, 0) + 1
        for sheet_name in workbook.sheetnames:
            try:
                sheet_info = load_sheet_info(workbook_path, sheet_name)
            except Exception:
                continue
            if sheet_info is None:
                continue

            duplicate_names = duplicate_headers(sheet_info.headers)
            join_keys = recommended_join_keys(sheet_info.workbook_name, sheet_name, sheet_info.headers)
            for column_name in sheet_info.headers:
                series = sheet_info.dataframe[column_name] if column_name in sheet_info.dataframe.columns else pd.Series(dtype="object")
                rows.append(
                    {
                        "Workbook Name": sheet_info.workbook_name,
